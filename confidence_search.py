...
def run_evaluation(args, model_args, data_args, training_args, bucket="mix", gt_path=None, use_abs_embedding=False):
    ...
    for itm in sampler_train.val_batch_sampling(batch_size=1, dataset=sampler_train.val_dataset):
        is_feature_match_spoiler = True # is_spoiler_type_match(itm)
        ...
        with torch.no_grad():
            predicted = model(input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask,
                              ...
                              using_modifier=using_modifier, lstm_span_modifier=lstm_span_modifier, use_abs_pos_embedding=use_abs_embedding)
    ...
    with open(os.path.join(LOADING_MODEL_PATH, "threshold_search_tmp_out_{0}.jsonl".format(bucket)), "w") as writer:
        writer.write(json.dumps(two_stage_results, indent=4) + "\n")

    if gt_path is not None and os.path.exists(gt_path):
        bucket_analyse_tmp_file = open(os.path.join(script_path, "tmp", bucket+"_predictions.txt"), encoding="utf8", mode="w")
...
def search_thresholds(args_parsed, gt_path=None):
    from nltk.translate.bleu_score import sentence_bleu
    import evaluate
    meteor = evaluate.load('meteor')

    assert gt_path is not None and os.path.exists(gt_path)
    val_inp = [json.loads(i) for i in open(gt_path, "r", encoding="utf8")]
    gt_mapping = {x["uuid"]: " ".join(x["spoiler"]) for x in val_inp}

    def is_spoiler_type_match(bucket, _itm):
        if bucket == "mix":
            return 1
        if "example_id" in _itm:
            feat_uuid = _itm["example_id"][0]
    ...
    predict_result_cache = {}
    buckets = ["phrase", "passage", "multi", "mix"]
    combined_predictions = {}
    for bucket in buckets:
        task_output_file = os.path.join(LOADING_MODEL_PATH, "threshold_search_tmp_out_{0}.jsonl".format(bucket))
        pred_outs = json.load(open(task_output_file, "r", encoding="utf8"))
        for pred_info in pred_outs:
            uuid = pred_info["uuid"]
            if uuid not in predict_result_cache:
                predict_result_cache[uuid] = {}
            predict_result_cache[uuid][bucket] = pred_info["spoiler"]

    threshold_ranges = [0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85]
    performances = []
    fw = open(os.path.join(script_path, "tmp", "threshold_performances.txt"), mode="w", encoding="utf8")
    for phrase_threshold in threshold_ranges:
        for passage_threshold in threshold_ranges:
            for multi_threshold in threshold_ranges:
                threshold_signature = (phrase_threshold, passage_threshold, multi_threshold)
                hypos, refs = [], []
                bleus = []
                for uuid in classification_prediction_map.keys():
                    if phrase_threshold <= classification_prediction_map[uuid]["confidences"][0]:
                        hypos.append(predict_result_cache[uuid]["phrase"])
                    elif passage_threshold <= classification_prediction_map[uuid]["confidences"][1]:
                        hypos.append(predict_result_cache[uuid]["passage"])
                    elif multi_threshold <= classification_prediction_map[uuid]["confidences"][2]:
                        hypos.append(predict_result_cache[uuid]["multi"])
                    else:
                        hypos.append(predict_result_cache[uuid]["mix"])
                    hypo_text = hypos[-1]
                    refs.append(gt_mapping[uuid])
                    _ref_splited = [[x for x in gt_mapping[uuid].split(" ") if len(x)>0]]
                    _hypo_splited = [x for x in hypo_text.split(" ") if len(x)>0]
                    bleu_len = min(len(gt_mapping[uuid].strip().split(" ")), 4)
                    weights = tuple([1.0 / bleu_len] * bleu_len + [0.0] * (4 - bleu_len))
                    bleu_score = sentence_bleu(_ref_splited, _hypo_splited, weights=weights)
                    bleus.append(bleu_score)
                # print(sum(bleus)/len(bleus))
                ...
                performances.append( ",".join([str(x) for x in threshold_signature]) + "\t"+ str(meteor_score)+"\t"+ str(sum(bleus)/len(bleus)))
                print(performances[-1])
                fw.write(performances[-1]+"\n")
                fw.close()
                fw = open(os.path.join(script_path, "tmp", "threshold_performances.txt"), mode="a", encoding="utf8")
...
if __name__ == "__main__":
    ...
    run_evaluation(args_parsed, model_args, data_args, training_args, bucket="phrase", gt_path=gt_path)
    time.sleep(1.0)
    ...
    time.sleep(1.0)
    search_thresholds(args_parsed, gt_path=gt_path)