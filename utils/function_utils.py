...
def get_optimizer_params_l(args, model, is_nested=True):
    ...
    if is_nested:
        ...
        inner_model = model
        # not_layer_group_params = [p for n, p in inner_model.named_parameters() if
        #                           not any(nd in n for nd in no_decay) and not any(nd in n for nd in group_all)]
    optimizer_grouped_parameters = [
        # 首先对所有层中需要权重衰减的need_decay参数设置weight_decay
        {'params': [p for n, p in inner_model.named_parameters() if not any(nd in n for nd in no_decay) and not any(nd in n for nd in group_all)],'weight_decay': args.weight_decay},
        # 对group1里的所有需要权重衰减的need_decay参数设置较低的学习率learning_rate/2.6
        {'params': [p for n, p in inner_model.named_parameters() if
        ...
        {'params': [p for n, p in inner_model.named_parameters() if any(nd in n for nd in no_decay) and not any(nd in n for nd in group_all)],'weight_decay': 0.0},
        # 对group1里的所有不需要权重衰减的no_decay参数设置较低的学习率learning_rate/2.6
        {'params': [p for n, p in inner_model.named_parameters() if any(nd in n for nd in no_decay) and any(nd in n for nd in group1)],'weight_decay': 0.0, 'lr': args.learning_rate/1.6},#/2.6
        # 对group2里的所有不需要权重衰减的no_decay参数设置中等的学习率learning_rate
        {'params': [p for n, p in inner_model.named_parameters() if any(nd in n for nd in no_decay) and any(nd in n for nd in group2)],'weight_decay': 0.0, 'lr': args.learning_rate},
        # 对group3里的所有不需要权重衰减的no_decay参数设置较高的学习率learning_rate * 2.6
        {'params': [p for n, p in inner_model.named_parameters() if any(nd in n for nd in no_decay) and any(nd in n for nd in group3)], 'weight_decay': 0.0, 'lr': args.learning_rate*1.6},#* 2.6
        # 对非transformer结构的Head层设置极高的学习率
        {'params': [] if not is_nested else [p for n, p in model.named_parameters() if "model" not in n], 'lr': args.learning_rate*4, "momentum": 0.99},#*10
    ]
    return optimizer_grouped_parameters


def get_optimizer_params_b(args, model, is_nested=True):
    """
    For Base Model
    """
    ...
    if is_nested:
        ...
        inner_model = model
        not_layer_group_params = [p for n,p in inner_model.named_parameters() if not any(nd in n for nd in no_decay) and not any(nd in n for nd in group_all)]
    optimizer_grouped_parameters = [
        # 首先对所有层中需要权重衰减的need_decay参数设置weight_decay
        {'params': [p for n, p in inner_model.named_parameters() if not any(nd in n for nd in no_decay) and not any(nd in n for nd in group_all)],'weight_decay': args.weight_decay},
        # 对group1里的所有需要权重衰减的need_decay参数设置较低的学习率learning_rate/2.6
        ...
        {'params': [p for n, p in inner_model.named_parameters() if any(nd in n for nd in no_decay) and not any(nd in n for nd in group_all)],'weight_decay': 0.0},
        # 对group1里的所有不需要权重衰减的no_decay参数设置较低的学习率learning_rate/2.6
        {'params': [p for n, p in inner_model.named_parameters() if any(nd in n for nd in no_decay) and any(nd in n for nd in group1)],'weight_decay': 0.0, 'lr': args.learning_rate/1.6},
        # 对group2里的所有不需要权重衰减的no_decay参数设置中等的学习率learning_rate
        {'params': [p for n, p in inner_model.named_parameters() if any(nd in n for nd in no_decay) and any(nd in n for nd in group2)],'weight_decay': 0.0, 'lr': args.learning_rate},
        # 对group3里的所有不需要权重衰减的no_decay参数设置较高的学习率learning_rate * 2.6
        {'params': [p for n, p in inner_model.named_parameters() if any(nd in n for nd in no_decay) and any(nd in n for nd in group3)], 'weight_decay': 0.0, 'lr': args.learning_rate * 1.6},
        # 对非transformer结构的Head层设置极高的学习率
        {'params': [] if not is_nested else [p for n, p in model.named_parameters() if "model" not in n], 'lr': args.learning_rate*10, "momentum": 0.99},
    ]

    return optimizer_grouped_parameters


def make_optimizer(args, model, is_model_nested=True):
    # 定义optimizer和各个层的学习率等参数
    optimizer_name = args.optimizer_type
    if args.hidden_size == 768:
        optimizer_grouped_parameters = get_optimizer_params_b(args, model, is_nested=is_model_nested)
    if args.hidden_size == 1024:
        optimizer_grouped_parameters = get_optimizer_params_l(args, model, is_nested=is_model_nested)
